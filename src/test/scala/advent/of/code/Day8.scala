...
class Day8 extends Day(8) {
  ...
  case class Node(children: Seq[Node], metaData: Seq[Int]) {
    val sum: Int = metaData.sum + children.map(_.sum).sum
    val value: Int = {
      if (children.isEmpty) metaData.sum
      else metaData.map(n => children.lift(n - 1).map(_.value).getOrElse(0)).sum
    }
  }
  ...
  def pNodeBody[_: P](h: NodeHeader): P[Node] = P(pChildren(h.childCount) ~ " " ~ pMetaData(h.metaDataCount)).map {
    case (c, m) => Node(c, m)
  }
